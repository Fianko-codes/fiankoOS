...
import { FileCode, ChevronRight, ChevronDown, File } from 'lucide-react';
...
const fileTree: FileTreeItem[] = [
  ...
  { name: 'resume.json', isFolder: false },
];
  ...
  return (
    <div className="h-full flex bg-ctp-crust">
      ...
      <div className="w-48 border-r border-ctp-surface0 overflow-auto">
        ...
        {fileTree.map((item) => (
          <TreeItem key={item.name} item={item} depth={0} onSelect={setSelectedFile} selected={selectedFile} />
        ))}
      </div>
      ...
      <div className="flex-1 flex flex-col">
        {/* Tab */}
        <div className="h-9 bg-ctp-mantle border-b border-ctp-surface0 flex items-center px-4">
